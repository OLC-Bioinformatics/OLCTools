...
import olctools.database_setup.settings
try:
    from olctools.database_setup.settings import ECOLI
except (NameError, ImportError):
    ECOLI = str()
try:
    from olctools.database_setup.settings import SENTERICA
except (NameError, ImportError):
    SENTERICA = str()
try:
    from olctools.database_setup.settings import YERSINIA
except (NameError, ImportError):
    YERSINIA = str()
    ...
    def api_keys(self):
        if self.organism == 'ecoli':
            ...
            if not self.api_key:
                ...
                with open(olctools.database_setup.settings.__file__, 'a+') as env:
